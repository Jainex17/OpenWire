  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: sectionId.includes("page-1")
          ? "rgba(26, 26, 46, 0.95)"
          : "rgba(255, 255, 255, 0.95)",
        borderBottom: sectionId.includes("page-1")
    ...
    >
      <div
        ...
        style={{ color: sectionId.includes("page-1") ? "#fff" : "#1a1a2e" }}
      >
        <span className="hover:outline-2 hover:outline-blue-500">
          <EditableText
            sectionId={sectionId}
            field="logo"
      ...
      <div className="flex gap-8 items-center">
        {activeDevice !== "mobile" &&
          ["Product", "Solutions", "Resources", "Pricing"].map(
            (item, index) => (
              <span
                key={item}
                className="text-sm font-medium cursor-pointer hover:opacity-80 transition-opacity hover:outline-2 hover:outline-blue-500"
                style={{
                  color: sectionId.includes("page-1") ? "#ccc" : "#4b5563",
                }}
              >
                <EditableText
                  sectionId={sectionId}
        ...
        {activeDevice === "mobile" ? (
          <div className="flex flex-col gap-1.5 cursor-pointer">
            <span
              className={`w-6 h-0.5 rounded-full ${sectionId.includes("page-1") ? "bg-white" : "bg-gray-800"}`}
            ></span>
            <span
              className={`w-6 h-0.5 rounded-full ${sectionId.includes("page-1") ? "bg-white" : "bg-gray-800"}`}
            ></span>
